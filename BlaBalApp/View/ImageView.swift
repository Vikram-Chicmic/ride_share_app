                    ...
                    StrokeShape(size: size)
                        ...
                        .frame(width: size, height: size).overlay(alignment: .center) {
                            if let image = selectedImage {
                                ...
                                Image(systemName: Constants.Icons.perosncircle).resizable().rotationEffect(Angle(degrees: -20)).clipShape(ImageShape(imageSize: size - (size/10))).frame(width: size - (size/10), height: size - (size/10)).foregroundColor(.white)
                            }
                        }
